...
It then fetch the data available for 10 second windows for 2 weeks starting from Feb 4,2013 for the time window of 8am to 5pm for all the users.
...
# # There are 3240 windows of 10 seconds created per day. So the above optimization reduced 5 * 3240 loops.
...
def create_time_slice(files,fname):
    # Array with the starting EPOCH of all the 10 days
    ...
    for t in range(5): # There are 5 days in a week
        ...
        for x in range(3240): # there are 3240 10 second windows in a day
            first1=start_val1+x*10000 # adding 10 sec
            second1=start_val1+(x+1)*10000 # adding 10 sec
            first2=start_val2+x*10000 # adding 10 sec
            second2=start_val2+(x+1)*10000 # adding 10 sec
        ...
        week2_df=week2_df.append(second_df) #adding the consolidated data of the day to the dataframe for week 1
        #print("Loop-->"+str(week2_df.shape))
    #print(week2_df.shape)
    week1_df=week1_df.append(week2_df) # appending week 2 data to week 1 data
    path='.\output\\'+fname+".csv"
